...
\Cref{c:background} is structured as follows:\\
...
Generally these methods can be categorized into three groups.
...
\subsection{Classical Segmentation}\label{s:b-seg-techniques}
...
\Cref{fig:kmeans1} displays a set of data-points, represented as black dots, and a small number of
...
If the merging of two regions is to be avoided, an additional watershed, called \textit{dam},
...
\section{Segmentation representation}\label{s:b-seg-rep}
\begin{figure}[h]
	\centerline{
...
\Cref{fig:segRep} gives an overview of the most common representations.
...
so that every voxel enc	ompassed by the mesh belongs to the segment \cite{delameMedialSurfaceMesh2012,weber3DSlicerPlatform2022,pinterPolymorphSegmentationRepresentation2019}.
...
As illustrated, the first corner intersects with the third corner at the point where $x = −1$ and $z = 1$.
The second corner intersects with the first only at position $z = 1$,
whereas it intersects with the third at both $y = 1$ and $z = 1$.
Ultimately, the third corner intersects with the first at the point $x = −1$ and with the second at the points $y = 1$ and $z = 1$.
...
\subsection{Pompeiu-Hausdorff Distance}
